...
const AdminSoccerTable = ({ sportId = 1 }) => {
  ...
  // If Redux matches changed, reset optimistic view
  useEffect(() => {
    setOptimisticMatches(soccerMatches);
    setSelectedMatchIds([]); // clear selection on update
  }, [soccerMatches]);
  ...
  // --- MAIN ADMIN FILTER: Only require 2 or more runners and a date ---
  const filteredMatches = (optimisticMatches.length > 0 ? optimisticMatches : soccerMatches).filter(
    (match) =>
      Array.isArray(match.matchRunners) &&
  ...
  if (!todayMatches.length && !tomorrowMatches.length && !liveMatches.length)
    return <div>No soccer matches found.</div>;
  ...
  return (
    <>
      ...
      <div className="container mt-4 ">
        <h1 className="mb-4">Soccer: Upcoming, Today, Tomorrow & Live Matches</h1>
